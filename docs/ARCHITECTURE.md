...
**داخل یک Interactive Transaction، بعد از `$executeRaw`، Query ای که به چند Statement تبدیل می‌شود (`include` روی چند Relation) باعث هشدار `client.query() while already executing` در `pg` می‌شود** — هشداری که در `pg@9` به خطا تبدیل خواهد شد. راه‌حل: داخل Transaction فقط ستون انتخاب کن و بعد از Commit دوباره بخوان.
