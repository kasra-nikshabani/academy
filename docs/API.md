...
| مسیر | متد | مجوز |
|---|---|---|
| `/api/v1/talent/pipeline` | `GET` | `tryout:read` |

پارامترها: `seasonId`، `sportId`، `tryoutId`، و `allSeasons=true`.

پیش‌فرض، **فصل جاری** است. `allSeasons=true` همه دوره‌هایی را می‌شمارد که آکادمی تا امروز برگزار کرده — عددی که گزارش سالانه می‌خواهد و برای مدیری که می‌خواهد بداند امروز چه کند بی‌فایده است.

پاسخ شامل `counts`، `stages` (چهار مرحله با سهم و تعداد کنار رفته)، `conversion`، `stillOpen`، `openTryouts`، `unfinishedEvaluations`، `acceptedWithoutEvaluation` و `queues` (سه صف، قدیمی‌ترین اول) است.
...
| مسیر | متد | محافظ |
|---|---|---|
| `/api/v1/players/:id/performance` | `GET` | `performance:read` + Scope **بازیکن** |
| `/api/v1/players/:id/performance` | `POST` | `performance:write` + Scope **بازیکن** |
| `/api/v1/teams/:id/performance` | `GET` | `performance:read` + Scope **تیم** |

خواندن و نوشتن یک بازیکن هر دو با Scope بازیکنی محدود می‌شوند و فقط با مجوز از هم جدا می‌شوند؛ ولی همان مسیر را می‌خواند و روی همان مسیر `403` می‌گیرد. نمای تیم با Scope تیمی محدود است، چون ترکیب تیم چیزی نیست که ولی ببیند.

**یک درخواست، یک روز تست:**

```json
{
  "measuredAt": "2026-09-15T00:00:00.000Z",
  "entries": [
    { "metric": "SPRINT_20M_S", "value": 3.28 },
    { "metric": "VERTICAL_JUMP_CM", "value": 41 }
  ]
}
```

`measuredAt` اختیاری است و پیش‌فرضش امروز است؛ به نیمه‌شب تهران در روز تست گرد می‌شود. ارسال دوباره همان سنجه در همان روز، مقدار را **اصلاح** می‌کند نه تکرار.

`entries` همه سنجه‌ها را لازم ندارد — جلسه‌ای که فقط دو تست گرفته، فقط دو ردیف می‌فرستد. هر سنجه در یک درخواست فقط یک بار.

هر مقدار با بازه **همان سنجه** سنجیده می‌شود: دوی ۲۰ متر در ۴۵ ثانیه و تست کوپر ۴۵ متری هر دو `422` می‌گیرند، و یک کران عددی مشترک هیچ‌کدام را نمی‌گرفت. تاریخ آینده هم `422` است.

پاسخ، پرونده کامل بازیکن است: `records` (همه اندازه‌گیری‌ها، قدیمی‌ترین اول) و `trends` (یکی برای هر سنجه، با `latest`، `delta`، `improved` و `best`). `improved` وقتی `null` است که سنجه جهت ندارد (قد، وزن) یا این اولین اندازه‌گیری است — و `null` هرگز به معنی «بدون تغییر» نیست.

### اعلان و اطلاعیه

| مسیر | متد | محافظ |
|---|---|---|
| `/api/v1/notifications` | `GET` | فقط صندوق **خودِ** فراخوان |
| `/api/v1/notifications/:id` | `PATCH` | خواندن یک اعلان خودِ فراخوان |
| `/api/v1/notifications/read-all` | `POST` | پاک کردن Badge |
| `/api/v1/announcements` | `GET` / `POST` | `notification:read` / `notification:send` + Scope |
| `/api/v1/announcements/:id` | `GET` | `notification:read` |
| `/api/v1/announcements/:id/publish` | `POST` | `notification:send` + Scope |

**صندوق شناسه ندارد.** هیچ‌کدام از سه مسیر اول راهی برای درخواست صندوق شخص دیگری ندارند؛ محدودسازی ساختاری است نه بررسی‌شده، پس بررسی‌ای هم نیست که فراموش شود. `PATCH` روی اعلان شخص دیگر و روی شناسه‌ای که وجود ندارد یک جواب می‌گیرند: `404`.

`GET /api/v1/notifications` شمارش خوانده‌نشده‌ها را در `meta.unread` هم برمی‌گرداند، چون هر فراخوانی که فهرست را می‌خواهد Badge را هم می‌خواهد.

**`POST` اطلاعیه، پیش‌نویس می‌سازد.** هیچ پرچم `publish: true` وجود ندارد؛ انتشار مسیر جداگانه‌ای است:
...
`teamId` و `schoolId` با هم پذیرفته نمی‌شوند؛ نبودن هر دو یعنی کل آکادمی، که فقط مدیر می‌تواند. مربی روی تیم خودش `201` می‌گیرد، روی تیم دیگر `403`، و روی کل آکادمی `422`.

`POST .../publish` **یک بار** کار می‌کند: بار دوم `409` است، نه ارسال دوم. پاسخ، اطلاعیه با `publishedAt` و `recipients` است.

### داشبورد

| مسیر | متد | محافظ |
|---|---|---|
| `/api/v1/dashboard` | `GET` | `report:read` **و** بدون Scope |

فقط نمای آکادمی Endpoint دارد. بخش تیم و بخش خانواده از سرویس‌هایی ساخته می‌شوند که Endpoint خودشان را دارند، و راه دومی برای پرسیدن همان سؤال یعنی جای دومی که قواعد Scope در آن واگرا شود.
